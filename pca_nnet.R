...
pca_nnet<-cbind(data_pca,response)
View(pca_nnet)
str(pca_nnet)


#creating data partition
index<-createDataPartition(pca_nnet$response,p=0.8)[[1]]
train_data<-pca_nnet[index,]
test_data<-pca_nnet[-index,]
...
nnet2_pca <- h2o.grid(
  ...
  grid_id="nnet2_pca_grid2", 
  model_id="nnet_pca_model_2", 
...
model_list <- h2o.getGrid("nnet2_pca_grid2",sort_by="auc",decreasing=TRUE)
model_list
...
## Sort by logloss
h2o.getGrid("nnet2_pca_grid2",sort_by="logloss",decreasing=FALSE)
