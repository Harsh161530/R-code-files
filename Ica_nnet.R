...
ica_nnet<-cbind(data_ica,response)
View(ica_nnet)
str(ica_nnet)


#creating data partition
index<-createDataPartition(ica_nnet$response,p=0.8)[[1]]
train_data<-ica_nnet[index,]
test_data<-ica_nnet[-index,]
...
nnet2_ica <- h2o.grid(
  ...
  grid_id="nnet2_ica_grid2", 
  model_id="nnet_ica_model_2", 
...
model_list <- h2o.getGrid("nnet2_ica_grid2",sort_by="auc",decreasing=TRUE)
model_list
...
## Sort by logloss
h2o.getGrid("nnet2_ica_grid2",sort_by="logloss",decreasing=FALSE)
