...
index<-createDataPartition(kmean_nnet_data$y,p=0.8)[[1]]
train_data<-kmean_nnet_data[index,]
test_data<-kmean_nnet_data[-index,]


response<-"y"
predictors<-setdiff(names(kmean_nnet_data),response)
predictors
...
nnet2_kmean <- h2o.grid(
  ...
  grid_id="nnet2_kmean_grid2", 
  model_id="nnet_kmean_model_2", 
...
model_list <- h2o.getGrid("nnet2_kmean_grid2",sort_by="auc",decreasing=TRUE)
model_list
...
## Sort by logloss
h2o.getGrid("nnet2_kmean_grid2",sort_by="logloss",decreasing=FALSE)
